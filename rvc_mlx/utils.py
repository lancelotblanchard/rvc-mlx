...
def narrow(
    ...
    if dim < -x.ndim or dim >= x.ndim:
        raise IndexError(f"Dimension out of range (expected to be in range of [{-x.ndim}, {x.ndim-1}], but got {dim}).")
    if length < 0:
    ...
    if start.item() + length > x.shape[dim]:
        raise ValueError(f"start ({start.item()}) + length ({length}) exceeds dimension size ({x.shape[dim]}).")
...
def pad_constant(
    ...
    if num_pad_dims > ndim:
        raise ValueError(f"Length of pad should be no more than twice the number of dimensions of the input. Pad length is {num_pad_dims} while the input has {ndim} dimensions.")
    ...
    for i in range(ndim-num_pad_dims, ndim):
        ...
        # Left
        if pad[pad_idx] < 0:
            new_input = narrow(new_input, dim=i, start=-pad[pad_idx], length=new_input.shape[i] + pad[pad_idx])
        elif pad[pad_idx] != 0:
            all_pads_non_positive = False

        # Right
        if pad[pad_idx + 1] < 0:
            new_input = narrow(new_input, dim=i, start=0, length=new_input.shape[i] + pad[pad_idx + 1])
    ...
    for i in range(num_pad_dims):
        ...
        if pad[pad_idx] > 0:
            start_indices.append(pad[pad_idx] if pad[pad_idx] >= 0 else input.shape[dim_idx] + pad[pad_idx])
            axes.append(dim_idx)
        elif pad[pad_idx + 1] > 0:
            start_indices.append(0)
            axes.append(dim_idx)

    out = mx.full(new_shape, value)
